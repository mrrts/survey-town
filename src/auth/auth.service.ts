...
export class AuthService {
  ...
  async login(loginDto: LoginDto, req: Request): Promise<IUser> {
    ...
    const user: IUser = await this.usersService.findByEmailAddress(
      emailAddress,
    );
  ...
  async register(dto: RegisterDto, req: Request): Promise<IUser> {
    const userByHandle: IUser = await this.usersService.findByHandle(
      dto.handle,
    );
    const userByEmail: IUser = await this.usersService.findByEmailAddress(
      dto.emailAddress,
    );
